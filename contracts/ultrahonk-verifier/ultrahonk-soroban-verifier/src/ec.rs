...
const INFINITY_BYTES: [u8; 64] = [0u8; 64];

/// Multi-scalar multiplication on G1: ∑ sᵢ·Cᵢ
///
/// Skips terms where the scalar is zero OR the commitment is the point at
/// infinity (all-zero bytes).  The latter avoids expensive `g1_mul` host calls
/// on dummy fold commitments that Shplemini inserts for unused log-N rounds.
#[inline(always)]
pub fn g1_msm(env: &Env, coms: &[G1Point], scalars: &[Fr]) -> Result<Bn254G1Affine, &'static str> {
    ...
    let mut acc = Bn254G1Affine::from_array(env, &INFINITY_BYTES);
    for (c, s) in coms.iter().zip(scalars.iter()) {
        if s.is_zero() || c.to_bytes() == INFINITY_BYTES {
            continue;
        }
