...
//! lookup, range, elliptic, memory, nnf, Poseidon external/internal) into a single
//! scalar which is then batched with the alpha challenges.
//!
//! bb v3.0.0: 28 subrelations (was 26). qAux split into qMemory + qNnf.
...
/// bb v3.0.0: Added read_tag boolean check at index 6.
fn accumulate_log_derivative_lookup_relation(
    ...
    // bb v3.0.0: read_tag boolean check
...
/// bb v3.0.0: Extracted from old auxiliary relation. Uses QMemory instead of QAux.
/// RAM subrelations now gated by Q_O instead of QArith.
fn accumulate_memory_relation(
    ...
    // Contribution 14: ROM adjacent values match
    ...
    // Contributions 16,17,18: RAM (gated by Q_O instead of QArith in bb v3.0.0)
...
/// Accumulate non-native field (NNF) subrelation (index 19).
/// bb v3.0.0: Extracted from old auxiliary relation. Uses QNnf instead of QAux.
